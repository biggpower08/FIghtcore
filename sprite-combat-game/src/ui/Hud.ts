...
export class Hud {
  draw(ctx: CanvasRenderingContext2D, player: Player, wave: number, boss: Boss | null): void {
    ...
    const moveText = player.equippedMoves.map((move, index) => `${index + 1}:${move.name}`).join('  ');
    ctx.fillText(`Moves ${moveText}`, 22, ctx.canvas.height - 36);
    ctx.fillText('J light  K heavy  L grapple  Space dash', 22, ctx.canvas.height - 62);
