...
const FRAME_SOURCES = [
  ...
  },
  {
    ...
    label: 'alpha-cleaned PNGs',
    priority: 3,
    registry: 'filesystem alpha-cleaned frames; metadata inherited from source frame',
    overwrite: 'Generated output. npm.cmd run sprites:clean-alpha may overwrite this folder.',
    safeManualEdit: false,
  },
  {
    id: 'frames-reference',
...
function printTable(items) {
  ...
  console.log('');
  console.log('Runtime priority: manual-overrides > frames-alpha-repaired > frames-cleaned > frames-pack > frames-reference > frames-semi-realistic > raw fallback.');
...
function renderMarkdown(items) {
  ...
  lines.push('');
  lines.push('Runtime priority: `manual-overrides` > `frames-alpha-repaired` > `frames-cleaned` > `frames-pack` > `frames-reference` > `frames-semi-realistic` > raw fallback.');
  lines.push('');
